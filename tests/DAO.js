describe('DAO tests', function() {
    ...
    beforeEach(inject(function(_itemsStorage_){
        itemsStorage = _itemsStorage_;
    }));
    ...
    it('should have structure', function() {
        expect(itemsStorage.getItem).toBeDefined();
        expect(itemsStorage.list).toBeDefined();
    });

    it('test get list', function(){
        var result;
        itemsStorage.list().then(function(data){
            result = data
        });

        $httpBackend.expectGET(function(url){ return url.indexOf(SOURCE)>=0 }).respond(200,EXAMPLE_DATA);
        $httpBackend.flush();

        expect(result.items.length).toEqual(EXAMPLE_DATA.items.length);
        expect(result.pageInfo).toEqual(EXAMPLE_DATA.pageInfo);
    });

    it('test get item', function(){
        var result, id = EXAMPLE_DATA.items[0].id;
        itemsStorage.getItem(id).then(function(data){
            result = data
        });

        var simpleExample = angular.copy(EXAMPLE_DATA);
        simpleExample.items = simpleExample.items.filter(function(itm){ return itm.id == id });

        $httpBackend
            .expectGET(function(url){ return url.indexOf(SOURCE)>=0 })
            .respond(200,simpleExample);
        $httpBackend.flush();
        expect(result.id).toEqual(id);
    });

    it('test get empty item', function(){
        var result;
        itemsStorage
            .getItem('undefined_id')
            .then(function(data){ result = data },function(e){ result = e });
        ...
        $httpBackend
            .expectGET(function(url){ return url.indexOf(SOURCE)>=0 })
            .respond(404,simpleExample);
        $httpBackend.flush();
        expect(result).toEqual('not found item undefined_id');
    });
